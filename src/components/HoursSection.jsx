...
const HoursSection = () => {
  return (
    <div className="relative group h-full flex flex-col">
      {/* 1. Subtle Ambient Glow: Controlled focus */}
      <div className="absolute inset-0 bg-champagne/5 blur-[80px] rounded-full opacity-0 group-hover:opacity-100 transition-opacity duration-1000 pointer-events-none" />

      <div className="flex-1 glass-gold p-8 md:p-10 rounded-[2.5rem] border border-white/5 relative overflow-hidden flex flex-col">
        {/* 2. Header with Status Badge */}
        <div className="flex items-start justify-between mb-10">
          <div>
            <span className="text-[10px] uppercase tracking-[0.3em] text-champagne font-bold mb-1 block">
              Timeline
            </span>
            <h3 className="text-2xl font-serif italic text-mist">
              Opening Hours
          ...
          <div className="flex items-center gap-2 px-3 py-1.5 rounded-full border border-white/10 bg-white/5 backdrop-blur-md">
            ...
            <span className="text-[9px] uppercase tracking-widest text-mist font-bold">
              Live
            </span>
          </div>
        </div>

        {/* 3. The Editorial List: Perfect Alignment */}
        <div className="space-y-5 relative z-10 flex-1 flex flex-col justify-center">
          {restaurant.hours.map((h, i) => (
            <div key={i} className="flex items-baseline group/line">
              {/* FIXED WIDTH FOR DAYS: This is the secret to professional alignment */}
              <span className="w-12 text-[11px] font-bold uppercase tracking-[0.2em] text-smoke group-hover/line:text-champagne transition-colors duration-300">
                {h.day}
              </span>

              {/* REFINED DOTTED LEADER */}
              <div className="flex-1 mx-4 border-b border-dotted border-white/20 mb-1 group-hover/line:border-champagne/40 transition-colors duration-300" />

              {/* TIME: Monospace for numeric clarity */}
              <span className="text-xs font-medium text-mist font-mono tracking-tighter">
                {h.time}
              </span>
            </div>
          ))}
        ...
        <div className="mt-8 pt-6 border-t border-white/5">
          <p className="text-[9px] text-center text-smoke/50 uppercase tracking-[0.25em] italic">
            * Late night service available on request
          </p>
        </div>

        {/* Subtle Decorative Background Element */}
        <div className="absolute -bottom-10 -right-10 opacity-[0.03] pointer-events-none text-champagne">
